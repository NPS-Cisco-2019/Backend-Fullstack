...
export function Setting({
  ...
  let Component;
  let height = window.innerHeight / 14 + window.innerHeight / 40;
  ...
  return (
    <div
      className={`setting-wrapper ${transition ? "height-trans" : null}`}
      id={`${id}-wrapper`}
    ...
    >
      <div
        className="setting"
        id={id}
        style={{ flexBasis: window.innerHeight / 12 }}
        onClick={openable ? openClick : null}
      >
        <p>{name}</p>
